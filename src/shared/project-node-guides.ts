import { getProjectNode, projectNodeLabel } from "./project-store.js"
...
/** 节点全文行：自定义优先，留空回退默认模板 */
export function getNodeGuideLines(id: string, customPrompt: string | undefined, label: string): string[] {
  const c = (customPrompt || "").trim();
  if (c) return c.split(/\r?\n/);
  return ACTION_GUIDES[id] ?? [`完成 ${label || id} 工作`];
}

/** 节点任务提示词全文组装（按钮与 project_get_node 同源；真值住下层） */
export function buildNodeActionPrompt(
  p: Pick<Project, "id" | "name" | "groupId">,
  nodeId: string,
  origin: string,
): string {
  const label = projectNodeLabel(nodeId, p.groupId)
  const guide = getNodeGuideLines(nodeId, getProjectNode(nodeId, p.groupId)?.prompt, label)
  return [
    ...
    "",
    "交付分级（按需，不硬造）:",
    "- 有实质文件产物时：写 AI 工作目录下的 .lk-harness/artifacts/，用 send_file 交付；需跨节点保留时调 project_register_artifact。",
    "- 无实质文件产物时（如纯评审结论、只问一句即答、只做状态确认）：以回复正文为交付，不造空文件、不硬登记。产物=文件；登记=跨节点上下文；报告=正文结论。",
    "",
    "收尾（每次必须）:",
    "1. 结论先行（一句话：干了什么、成没成）。",
    "2. 产物链接（如有：文件/MR/飞书文档）。",
    "3. 遗留事项与下一步（如无写“无”）。",
  ].join("\n")
}