...
/** Agent 资源：CLI（唯一�? SDK Key + 大模型供应商实例 */
...
export interface MessageChannel {
  ...
  // Agent 绑定
  agentResourceId: string;        // "cli" �?sdk 资源 id
  model: string;                  // 主模型（"" / "auto" = 默认�?
  modelParams: string;            // JSON 序列化的 {id,value}[]，仅 SDK
  othersModel: string;            // 其他�?群聊模型，空 = 跟随主模�?
...
export interface DaemonChannelConfig {
  ...
  /** 通道级工作目录，�?= 跟随全局 WORKSPACE_DIR */
  workspaceDir: string;
  /** 合成开关（keepSession && persistentPoll）：poll 响应随路下发，作�?Agent 收尾方式的权威来�?*/
  keepAlive?: boolean;
  /** 是否展示流式进度卡（默认 true�?*/
  showThinking?: boolean;
  /** 流式卡思�?工具块各保留最�?N 个（默认 5�?*/
  streamKeepPerKind?: number;
  /** 完整回复后隐藏思�?工具/todos 折叠块（默认 true�?*/
  hideThinkingOnFinish?: boolean;
}
...
/** �?sessionKey 提取 `::` 后缀的工作目录；仅路径形态有效（排除 wf_xxx 等非路径后缀�?*/
...
/**
 * 规范化会�?key：盘符路径上的重复反斜杠压成单个（防 JSON/环境变量双重转义导致队列目录分裂）�?
 * 特殊后缀（wf_/project_/�?temp_ 等）不动�?
 */
