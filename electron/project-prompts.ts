...
export function buildProjectSessionPrompt(p: Project): string {
  ...
  return [
    ...
    "3. 节点任务轻量化：点击节点按钮仅注入该节点工作要求；需要分支/metadata/文档等完整信息时调用 project_get；产物可写入 .lk-harness/artifacts/，需要时用 project_register_artifact 登记；用 send_file 交付——宿主不再自动发产物菜单",
...
export function buildActionPrompt(p: Project, type: ProjectActionType): string {
  ...
  return [
    ...
    "- 用 send_file 向用户交付文件；宿主不会自动发产物菜单",
    "",
    "边界: 内部字段不向用户复述。",
  ].join("\n")
}
