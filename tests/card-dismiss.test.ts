...
describe("指令卡关�?, () => {
  it("有按钮的指令卡（offerDismiss）自动追加关闭按�?, () => {
  ...
  it("sections 指令卡在最后一段追加关�?, () => {
    const card = LarkSender.buildCard("ignored", { title: "状�? }, undefined, undefined, undefined, undefined, [
      { text: "�?", buttons: [{ label: "A", value: { kind: "cmd", cmd: "/a" } }] },
      { text: "�?", buttons: [{ label: "B", value: { kind: "cmd", cmd: "/b" } }] },
    ], true) as { body: { elements: unknown[] } }
    const json = JSON.stringify(card)
    expect(json).toContain("�?关闭")
  })

  it("普�?Agent 回复卡（默认）不追加关闭按钮", () => {
    const card = LarkSender.buildCard("拼音小班课有效课节不�?0节查询�?, { title: "📂 lk-harness", subtitle: "🌿 main" }) as { body: { elements: unknown[] } }
    const json = JSON.stringify(card)
  ...
  it("已关闭卡片不再追加按�?, () => {
    ...
    const json = JSON.stringify(card)
    expect(json).toContain("菜单已关�?)
    expect(json).not.toContain("�?关闭")
  })

  it("无按钮的指令卡（offerDismiss）也有关闭按�?, () => {
    const card = LarkSender.buildCard("�?定时任务一�?, { title: "定时任务", subtitle: "列表" }, undefined, undefined, undefined, undefined, undefined, true) as { body: { elements: unknown[] } }
    const json = JSON.stringify(card)
    expect(json).toContain("�?关闭")
    expect(json).toContain('"kind":"dismiss"')
  })
